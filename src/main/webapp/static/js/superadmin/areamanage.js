...
function optFormater(value, row, index) {
	...
	var edit = '<a href="javascript:openDialog_edit(' + params + ')">编辑</a>' +'&nbsp;&nbsp;&nbsp;&nbsp;'+ '<a href="javascript:doDel(' + row.areaId + ')">删除</a>';
	return edit;
};
...
// 打开对话框
function openDialog_edit(areaId, areaName, priority) {
	areaManagementEditReset(areaId, areaName, priority);
...
// 根据用户id查询用户的信息
function areaManagementEditReset(areaId, areaName, priority) {
...
function doDel(areaId) {
	$.messager.confirm('删除提示', '你确定永久删除该数据吗?', function(r) {
		if (r) {
			var url = '/hoc/superadmin/removearea?areaId=' + areaId;
			changeStatus(url);
		}
	});
}
